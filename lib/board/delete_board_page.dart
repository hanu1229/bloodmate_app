...
class _DeleteBoardPageState extends State<DeleteBoardPage> {
  ...
  Future<void> deletePost() async {
    try {
      ...
      final response = await dio.delete("$domain/board/${widget.boardPostId}", options : Options(headers : {"Authorization" : token}));
      if(response.statusCode == 200) {
        await showDialog(
          context : context,
          builder : (context) => CustomAlertDialog(context : context, title : "삭제하기", content : "댓글을 정상적으로 삭제했습니다.", isChange : false),
        );
        Navigator.pop(context, widget.boardPostId);
      }
    ...
    return AlertDialog(
      ...
      content : Column(
        ...
        children : [
          ...
          Text("※ 게시물이 영구 삭제되어 복구 할 수 없습니다.", style : TextStyle(color : Colors.red, fontSize : 24, fontWeight : FontWeight.bold)),
        ],
      ),
      actions : [
        ElevatedButton(
