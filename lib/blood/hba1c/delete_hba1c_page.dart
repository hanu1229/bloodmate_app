...
class DeleteHba1cPage extends StatefulWidget {
  final int hba1cId;
  const DeleteHba1cPage({super.key, required this.hba1cId});

  @override
  State<DeleteHba1cPage> createState() => _DeleteHba1cPageState();
}

class _DeleteHba1cPageState extends State<DeleteHba1cPage> {
  ...
  Future<void> deleteData() async {
    try {
      ...
      final response = await dio.delete("$domain/blood/hba1c/${widget.hba1cId}", options : Options(headers : {"Authorization" : token}));
      if(response.statusCode == 200) {
        await showDialog(
          context : context,
          builder : (context) => CustomAlertDialog(context : context, title : "삭제하기", content : "삭제에 성공했습니다.", isChange : false),
        );
        Navigator.pop(context, widget.hba1cId);
      }
